...
@test "SHARKRITE_LINT_ONLY: exact-number match — 1 does not alias rules 10-19" {
  # Rule 1 exists (GREP_C_ECHO_ZERO). Scoping to 1 must not run rule 15:
  # plant a Rule 15 violation; scoped-to-1 run must NOT report it.
  cat > "$FIXTURE_DIR/bad.sh" <<'EOF'
#!/bin/bash
if echo "$X" | grep -q "sharkrite-parent-pr:"; then echo hi; fi
EOF
  RITE_LINT_EXTRA_DIRS="$FIXTURE_DIR" SHARKRITE_LINT_ONLY=1 run bash "$LINT_SCRIPT"
  [[ ! "$output" =~ "BARE_MARKER_GREP" ]]
}

@test "SHARKRITE_LINT_ONLY: empty value behaves as unset (all rules run)" {
  cat > "$FIXTURE_DIR/pipe.sh" <<'EOF'
#!/bin/bash
set -euo pipefail
VAR=$(echo "$text" | grep "pattern")
EOF
  # Empty knob → full scan → Rule 08 fires on the fixture.
  RITE_LINT_EXTRA_DIRS="$FIXTURE_DIR" SHARKRITE_LINT_ONLY= run bash "$LINT_SCRIPT"
  [ "$status" -eq 1 ]
  [[ "$output" =~ "UNSAFE_PIPE_IN_CMDSUB" ]]
}
