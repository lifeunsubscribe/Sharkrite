...
3. **Review/Assess Loop** — Generate review + run post-commit gate in parallel, assess combined findings (review + gate), fix ACTIONABLE_NOW items (up to 3 retries)
   ...
   - **Baseline-diff (new-failures-only):** the gate blocks/feeds the fix loop on test failures **this change introduced**, not on the pre-existing red baseline on `origin/main`. A failing targeted run re-runs only the failing-containing files at the diff base in a throwaway worktree (cached per base SHA; green runs pay nothing) and suppresses pre-existing reds. `outcome=passed ⟺ zero new failures`. Fail-safe: any error flags all failures; `RITE_GATE_BASELINE_DIFF=false` disables. See `behavioral-design.md` → "Gate Baseline-Diff: block on NEW failures only".
