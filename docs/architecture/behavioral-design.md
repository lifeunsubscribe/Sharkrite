...
2. **post-merge-verify's main-broken full-suite check was REMOVED.** It used to re-run the *entire* suite on `origin/main` when the post-merge gate failed, to ask "is main broken vs. did this merge break it?" That is now **redundant**: baseline-diff (#699) classifies every failure as new (since `pre_merge_ref` — the merge's own) vs pre-existing, and the gate's `outcome=failed` fires **only** when new>0. So a post-merge gate failure IS the merge's doing → `return 1` directly. It was also the **last full-suite run in the lifecycle** and the source of the flake cascade. The caller already handles `return 1` (revert), so removal is contained.
