...
| `15` | Number refers to a PR, not an issue. `handle_pr_number_refused()` printed the named-PR error and suggested the linked issue (if found). No dev session ran. **Single-issue mode exits 1** (the refusal message was already printed; `run_workflow` converts the 15 to 1 so callers in set -e chains don't see a surprising non-standard code). **Batch mode exits 15** so `batch-process-issues.sh` can record `pr_number_refused` (SKIPPED class) and skip post-issue stat-gathering. See: `handle_pr_number_refused()` in `workflow-runner.sh` and `tests/regression/pr-number-refused-as-issue.bats`. |
