...
| `18` | `lib/providers/claude.sh` (`claude_provider_run_agentic_session`), `lib/core/claude-workflow.sh` | `workflow-runner.sh` (propagates), `batch-process-issues.sh` | Provider auth failure — Claude CLI is logged out (401 / "Invalid API key · Please run /login" class error). Detected by fingerprinting the agentic session's stdout + stderr. A logged-out provider is batch-fatal: every subsequent issue will fail identically. **Single-issue mode:** exits 18 with a "run: claude /login" remediation message. **Batch mode:** halts immediately, marks remaining unprocessed issues `skipped:auth` (SKIPPED class). See: `tests/regression/batch-auth-halt.bats`. |
...
| `18` | Provider auth failure — Claude CLI logged out (401 / "Invalid API key" class). Propagated from `claude_provider_run_agentic_session()`. Single-issue: prints "run: claude /login" and exits 18. Batch: `batch-process-issues.sh` halts immediately and marks remaining issues `skipped:auth`. |
...
| `15` | Number refers to a PR, not an issue. `handle_pr_number_refused()` printed the named-PR error and suggested the linked issue (if found). No dev session ran. **Both single-issue and batch mode exit 15.** `run_workflow()` returns 15 in both modes; `main()` propagates it unconditionally. Batch: `batch-process-issues.sh` captures 15 to record `pr_number_refused` (SKIPPED class) and skip post-issue stat-gathering. Single: exit 15 is non-zero (refusal accepted) and avoids the misleading "Workflow failed" message that the generic else branch would print. See: `handle_pr_number_refused()` in `workflow-runner.sh` and `tests/regression/pr-number-refused-as-issue.bats`. |
| `18` | Provider auth failure — propagated from `claude-workflow.sh` (which got it from `claude_provider_run_agentic_session`). Batch: `batch-process-issues.sh` halts immediately and marks remaining issues `skipped:auth`. Single-issue: exits 18 with remediation message. |
...
| `17` | Batch refused — another batch is already running against this repo (repo-level mutex held by a live process). The error message names the holder PID and its issue list. Wait for the holder to finish, or kill the holder PID if it crashed and the lock is stale (stale locks are auto-reclaimed on the next invocation). See: issue #833. |
