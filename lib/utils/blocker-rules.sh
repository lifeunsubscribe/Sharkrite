...
# Source notifications library and gh retry helper
...
detect_infrastructure_changes() {
  ...
  local infra_files=$(gh_safe pr view "$pr_number" --json files --jq ".files[] | select(.path | test(\"${infra_pattern}\")) | .path" || echo "")
...
detect_database_migrations() {
  ...
  local migration_files=$(gh_safe pr view "$pr_number" --json files --jq ".files[] | select(.path | test(\"${migration_pattern}\")) | .path" || echo "")
  ...
  if [ -n "$migration_files" ]; then
    ...
    echo ""
    echo "Migration diff:"
    gh_safe pr diff "$pr_number" | head -50 || true
...
detect_auth_changes() {
  ...
  local auth_files=$(gh_safe pr view "$pr_number" --json files --jq ".files[] | select(.path | test(\"${auth_pattern}\")) | select(.path | test(\"tests?/|docs?/\") | not) | .path" || echo "")
...
detect_doc_changes() {
  ...
  local doc_files=$(gh_safe pr view "$pr_number" --json files --jq ".files[] | select(.path | test(\"${doc_pattern}\")) | .path" || echo "")
...
detect_critical_issues() {
  ...
  local review=$(gh_safe pr view "$pr_number" --json comments --jq '[.comments[] | select(.author.login | test("claude|github-actions"; "i"))] | .[-1] | .body' || echo "")
...
detect_expensive_services() {
  ...
  local expensive=$(gh_safe pr diff "$pr_number" | grep -iE "$expensive_pattern" || echo "")
...
detect_protected_scripts() {
  ...
  for script in "${protected_scripts[@]}"; do
    local changes=$(gh_safe pr view "$pr_number" --json files --jq ".files[] | select(.path | contains(\"$script\")) | .path" || echo "")
    ...
    if [ -n "$changes" ]; then
      ...
      echo ""
      echo "Diff:"
      gh_safe pr diff "$pr_number" -- "*/$script" | head -100 || true
      return 1
    fi
  done
