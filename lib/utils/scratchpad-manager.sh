...
# Do NOT export _RITE_SCRATCHPAD_MANAGER_LOADED — subprocesses must re-source.
if [ "${_RITE_SCRATCHPAD_MANAGER_LOADED:-}" = "true" ]; then
...
clear_current_work() {
  ...
  # Remove content between "## Current Work" and next "##", then add placeholder.
  # Uses awk for portability (BSD sed chokes on nested brace commands).
  awk '
    /^## Current Work/ {
      print
      print ""
      print "_No active work — run `rite <issue>` to start_"
      print ""
      skip = 1
      next
    }
...
create_tech_debt_issues() {
  ...
  while IFS= read -r entry; do
    ...
    # contain ${location}, so searching `in:title` for the location never matched
    ...
    # shell errors from propagating to stderr under set -euo pipefail).
    local _dedup_hash
    if command -v md5sum >/dev/null 2>&1; then
      # GNU/Linux: md5sum outputs "HASH  -"; take first 12 hex chars
      _dedup_hash=$(printf '%s' "$_dedup_key" | md5sum | cut -c1-12)
    elif command -v md5 >/dev/null 2>&1; then
      # BSD/macOS: md5 -q outputs just "HASH"; take first 12 hex chars
...
EOF
    ...
    # Create the issue (via temp file to avoid shell metacharacter issues in body)
    local body_file
    body_file=$(mktemp)
