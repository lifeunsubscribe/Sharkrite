...
repo_wide_status() {
  ...
  open_issues_json=$(gh_safe issue list --state open --json number,title,labels,createdAt --limit 200 2>/dev/null || echo "[]")
  ...
  local open_prs_json
  open_prs_json=$(gh_safe pr list --state open --json number,body,headRefName --limit 200 2>/dev/null || echo "[]")
  ...
  if [ "$open_count" -eq 0 ]; then
    ...
    while IFS= read -r issue_json; do
      ...
      if [ -n "$matched_pr" ] && [ "$matched_pr" != "null" ]; then
        ...
        pr_comments=$(gh_safe pr view "$pr_num" --json comments --jq '.comments' 2>/dev/null || echo "[]")
      fi
...
"
  ...
  closed_issues_json=$(gh_safe issue list --state closed --json number,title,labels,closedAt --limit 5 2>/dev/null || echo "[]")
  ...
  merged_prs_json=$(gh_safe pr list --state merged --json number,body --limit 20 2>/dev/null || echo "[]")
