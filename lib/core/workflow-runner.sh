...
phase_claude_workflow() {
  ...
  if [ "$RESUME_MODE" = true ]; then
    ...
    pr_number=$(gh_safe pr list --state open --json number,body --limit 100 2>/dev/null | \
...
<!-- sharkrite-auto-resolved -->" 2>/dev/null || true
      ...
      if [ $workflow_exit -eq 3 ]; then
        ...
        if [ $workflow_exit -eq 4 ]; then
          ...
          if [ -n "${PR_NUMBER:-}" ]; then
            local _pr_additions
            _pr_additions=$(gh_safe pr view "$PR_NUMBER" --json additions --jq '.additions' 2>/dev/null || echo "0")
...
phase_create_pr() {
  ...
  PR_NUMBER=$(gh_safe pr list --head "$branch_name" --json number --jq '.[0].number' 2>/dev/null || echo "")
...
phase_assess_and_resolve() {
  ...
  if [ "$retry_count" -eq 0 ]; then
    ...
    local pr_assess_state=$(gh_safe pr view "$pr_number" --json comments --jq '{
  ...
  local followup_marker=$(gh_safe pr view "$pr_number" --json comments --jq '.comments[].body' 2>/dev/null | grep -oE 'sharkrite-followup-issue:[0-9]+' | tail -1 || echo "")
  if [ -n "$followup_marker" ]; then
    local followup_issue_num=$(echo "$followup_marker" | cut -d: -f2)
    local followup_state=$(gh_safe issue view "$followup_issue_num" --json state --jq '.state' 2>/dev/null || echo "")
  ...
  if [ $assessment_result -eq 2 ]; then
    ...
    if [ -n "$review_content" ]; then
      ...
      if [ $fix_result -eq 3 ]; then
        ...
        echo "     gh_safe pr view $pr_number --json comments --jq '[.comments[] | select(.body | contains(\"sharkrite-assessment\"))] | .[-1].body'"
...
phase_merge_pr() {
  ...
  local _pr_info=$(gh_safe pr view "$pr_number" --json title,body 2>/dev/null || echo "{}")
  ...
  if [ -n "$_pr_title" ]; then
    ...
    if [ -n "$_summary" ]; then
      ...
      local _changed_files=$(gh_safe pr view "$pr_number" --json files --jq '.files[].path' 2>/dev/null || echo "")
      local _file_count=$(echo "$_changed_files" | grep -c '.' || true)
      local _commit_count=$(gh_safe pr view "$pr_number" --json commits --jq '.commits | length' 2>/dev/null || echo "?")
  ...
  _merged_branch=$(gh_safe pr view "$pr_number" --json headRefName --jq '.headRefName' 2>/dev/null || echo "")
...
phase_completion() {
  ...
  local pr_title=$(gh_safe pr view "$pr_number" --json title --jq '.title' 2>/dev/null || echo "Unknown")
  local files_changed=$(gh_safe pr view "$pr_number" --json files --jq '.files | length' 2>/dev/null || echo "?")

  # Check if follow-up issues were created
  local followup_issues=$(gh_safe issue list --label "follow-up" --label "parent:#${issue_number}" --json number --jq '. | length' 2>/dev/null || echo "0")
...
run_workflow() {
  ...
  local issue_data=$(gh_safe issue view "$issue_number" --json state,title,closedAt,closedByPullRequestsReferences 2>/dev/null)
  ...
  if [ "$issue_state" = "CLOSED" ]; then
    ...
    if [ -n "$pr_number" ]; then
      local pr_data=$(gh_safe pr view "$pr_number" --json state,mergedAt,body,headRefName 2>/dev/null)
    ...
    if [ -z "$pr_branch" ]; then
      ...
      closed_pr_number=$(gh_safe pr list --state closed --json number,body --limit 50 2>/dev/null | \
      ...
      if [ -n "$closed_pr_number" ]; then
        pr_branch=$(gh_safe pr view "$closed_pr_number" --json headRefName --jq '.headRefName' 2>/dev/null || echo "")
  ...
  if [ -z "${NORMALIZED_SUBJECT:-}" ]; then
    local _issue_json
    _issue_json=$(gh_safe issue view "$issue_number" --json title,body 2>/dev/null || echo "")
  ...
  if [ -z "${PR_NUMBER:-}" ] || [ "${PR_NUMBER:-}" = "null" ]; then
    ...
    local _detected_pr=$(gh_safe pr list --state open --json number,body --limit 100 2>/dev/null | \
      jq --arg issue "$issue_number" -r '.[] | select(.body | test("(Closes|closes|Fixes|fixes|Resolves|resolves) #" + $issue + "\\b")) | .number' | \
      head -1)
    ...
    if { [ -z "$_detected_pr" ] || [ "$_detected_pr" = "null" ]; } && [ -n "${WORKTREE_PATH:-}" ] && [ -d "${WORKTREE_PATH:-}" ]; then
      ...
      if [ -n "$_branch" ]; then
        _detected_pr=$(gh_safe pr list --head "$_branch" --json number --jq '.[0].number' 2>/dev/null || echo "")
      fi
    fi
  ...
  if [ -n "${PR_NUMBER:-}" ] && [ "${PR_NUMBER:-}" != "null" ]; then
    if [ -z "${WORKTREE_PATH:-}" ] || [ ! -d "${WORKTREE_PATH:-}" ]; then
      local _pr_branch=$(gh_safe pr view "$PR_NUMBER" --json headRefName --jq '.headRefName' 2>/dev/null || echo "")
  ...
  if [ -n "${PR_NUMBER:-}" ] && [ "$PR_NUMBER" != "null" ] && [ -n "${WORKTREE_PATH:-}" ] && [ -d "$WORKTREE_PATH" ]; then
    ...
    local pr_state_json=$(gh_safe pr view "$PR_NUMBER" --json comments --jq '{
