...
if [ -z "$PR_NUMBER" ]; then
  PR_NUMBER=$(gh_safe pr list --state all --json number,body --limit 100 2>/dev/null | \
...
if [ -z "$PR_NUMBER" ]; then
  ISSUE_TITLE=$(gh_safe issue view "$ISSUE_NUMBER" --json title --jq '.title' 2>/dev/null || echo "")
  if [ -n "$ISSUE_TITLE" ] && [ "$ISSUE_TITLE" != "null" ]; then
    PR_NUMBER=$(gh_safe pr list --state all --json number,title --limit 100 2>/dev/null | \
...
if [ -n "$PR_NUMBER" ]; then
  PR_DATA=$(gh_safe pr view "$PR_NUMBER" --json state,headRefName,mergedAt 2>/dev/null || echo "")
  if [ -n "$PR_DATA" ]; then
...
if [ -n "$PR_NUMBER" ]; then
  ...
  done < <(gh_safe issue list --state all --search "sharkrite-parent-pr:$PR_NUMBER in:body" --json number --jq '.[].number' 2>/dev/null || echo "")

  # Method 2: PR comment markers <!-- sharkrite-followup-issue:N -->
  while IFS= read -r num; do
    [ -n "$num" ] && FOLLOWUP_ISSUES+=("$num")
  done < <(gh_safe pr view "$PR_NUMBER" --json comments --jq '.comments[].body' 2>/dev/null | \
...
ISSUE_STATE=""
ISSUE_TITLE=""
ISSUE_DATA=$(gh_safe issue view "$ISSUE_NUMBER" --json state,title 2>/dev/null || echo "")
...
if [ ${#FOLLOWUP_ISSUES[@]} -gt 0 ]; then
  ...
  for issue_num in "${FOLLOWUP_ISSUES[@]}"; do
    local_state=$(gh_safe issue view "$issue_num" --json state --jq '.state' 2>/dev/null || echo "")
