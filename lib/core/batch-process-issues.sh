...
for ISSUE_NUM in "${ISSUE_LIST[@]}"; do
  ...
  # Fetch issue details
  ISSUE_DETAILS=$(gh_safe issue view "$ISSUE_NUM" --json title,labels,state 2>/dev/null || echo "{}")
  ...
  ISSUE_BODY=$(gh_safe issue view "$ISSUE_NUM" --json body --jq '.body' 2>/dev/null || echo "")
  PARENT_PR=""
  ...
  if echo "$ISSUE_BODY" | grep -qE "sharkrite-parent-pr:[0-9]+"; then
    ...
    if [ -n "$PARENT_PR" ]; then
      ...
      PARENT_PR_STATE=$(gh_safe pr view "$PARENT_PR" --json state --jq '.state' 2>/dev/null || echo "")

      if [ "$PARENT_PR_STATE" = "OPEN" ]; then
        # Check if parent issue is also in this batch (deliberate pairing)
        PARENT_ISSUE=$(gh_safe pr view "$PARENT_PR" --json body --jq '.body' 2>/dev/null | grep -oE 'Closes #[0-9]+' | head -1 | grep -oE '[0-9]+' || echo "")
  ...
  if [ -n "$DEP_ISSUES" ]; then
    ...
    for dep_num in $DEP_ISSUES; do
      ...
      dep_issue_state=$(gh_safe issue view "$dep_num" --json state --jq '.state' 2>/dev/null || echo "")
  ...
  EXISTING_PR=""
  for pr_num in $(gh_safe pr list --state open --json number --jq '.[].number' 2>/dev/null); do
    if gh_safe pr view "$pr_num" --json body --jq '.body' 2>/dev/null | grep -q "Closes #${ISSUE_NUM}\$\|Closes #${ISSUE_NUM}[^0-9]"; then
      EXISTING_PR="$pr_num"
      break
    fi
  done
  ...
  if [ -n "$EXISTING_PR" ]; then
    ...
    if [ "$SMART_WAIT" = true ]; then
      ...
      REVIEW_TIME=$(gh_safe pr view "$EXISTING_PR" --json comments --jq '.comments | map(select(.author.login == "claude")) | .[-1].createdAt' 2>/dev/null || echo "")
      ...
      if [ -n "$PR_UPDATED" ] && [ -n "$REVIEW_TIME" ] && [[ "$PR_UPDATED" > "$REVIEW_TIME" ]]; then
        ...
        while true; do
          ...
          NEW_REVIEW_TIME=$(gh_safe pr view "$EXISTING_PR" --json comments --jq '.comments | map(select(.author.login == "claude")) | .[-1].createdAt' 2>/dev/null || echo "")
    ...
    PR_BRANCH=$(gh_safe pr view "$EXISTING_PR" --json headRefName --jq '.headRefName' 2>/dev/null || echo "")
  ...
  if "$RITE_LIB_DIR/core/workflow-runner.sh" "$ISSUE_NUM" --unsupervised; then
    ...
    PR_NUMBER=$(gh_safe pr list --search "fixes #${ISSUE_NUM} OR closes #${ISSUE_NUM} in:body" --state all --json number --jq 'sort_by(.number) | reverse | .[0].number' 2>/dev/null || echo "")
    ...
    if [ -n "$PR_NUMBER" ]; then
      ...
      BRANCH_NAME=$(gh_safe pr view "$PR_NUMBER" --json headRefName --jq '.headRefName' 2>/dev/null || echo "")
      if [ -n "$BRANCH_NAME" ]; then
        ISSUE_BRANCH["$ISSUE_NUM"]="$BRANCH_NAME"
      fi

      # Capture changes summary (files changed + lines)
      PR_STATS=$(gh_safe pr view "$PR_NUMBER" --json additions,deletions,changedFiles --jq '"\(.changedFiles) files, +\(.additions)/-\(.deletions) lines"' 2>/dev/null || echo "")
      if [ -n "$PR_STATS" ]; then
        PR_CHANGES["$PR_NUMBER"]="$PR_STATS"
      fi

      # Check for security doc updates
      SECURITY_DOC_UPDATED=$(gh_safe pr view "$PR_NUMBER" --json files --jq '.files[].path' 2>/dev/null | grep -c "docs/security/DEVELOPMENT-GUIDE.md" || true)
      ...
      NEW_DEBT_ISSUE=$(gh_safe issue list --label "tech-debt" --state open --search "sharkrite-parent-pr:$PR_NUMBER in:body" --json number --jq '.[0].number' 2>/dev/null || echo "")
...
if [ ${#BLOCKED_ISSUES[@]} -gt 0 ]; then
  ...
  for ISSUE_NUM in "${BLOCKED_ISSUES[@]}"; do
    ...
    FOLLOWUP_ISSUE=$(gh_safe issue list --search "parent-pr in:body in:title" --label "review-follow-up" --state open --json number,body --jq ".[] | select(.body | contains(\"#$ISSUE_NUM\")) | .number" 2>/dev/null | head -1 || echo "")
