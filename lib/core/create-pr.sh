...
EXISTING_PR=$(gh_safe pr list --head "$CURRENT_BRANCH" --json number,title,url,state,isDraft --jq '.[0]' 2>/dev/null || echo "")
...
if [ ! -z "$EXISTING_PR" ] && [ "$EXISTING_PR" != "null" ]; then
  ...
  EXISTING_BODY=$(gh_safe pr view "$PR_NUMBER" --json body --jq '.body' 2>/dev/null || echo "")
...
if [ "$PR_EXISTS" = false ]; then
  ...
  if [ ! -z "$ISSUE_NUMBER" ]; then
    ...
    ISSUE_JSON=$(gh_safe issue view $ISSUE_NUMBER --json title,body,labels 2>/dev/null || echo "")
