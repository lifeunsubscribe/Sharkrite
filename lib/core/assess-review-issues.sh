...
check_assessment_freshness() {
  ...
  assessment_timestamp=$(gh_safe pr view "$pr_number" --json comments \
    --jq '[.comments[] | select(.body | contains("<!-- sharkrite-assessment"))] | sort_by(.createdAt) | reverse | .[0].createdAt' || echo "")
  ...
  assessment_body=$(gh_safe pr view "$pr_number" --json comments \
    --jq '[.comments[] | select(.body | contains("<!-- sharkrite-assessment"))] | sort_by(.createdAt) | reverse | .[0].body' || echo "")
...
ISSUE_CONTEXT=$(gh_safe pr view "$PR_NUMBER" --json body --jq '.body' | grep -oE 'Closes #[0-9]+|Fixes #[0-9]+|Resolves #[0-9]+' | head -1 | grep -oE '[0-9]+' || echo "")
if [ -n "$ISSUE_CONTEXT" ]; then
  ISSUE_DETAILS=$(gh_safe issue view "$ISSUE_CONTEXT" --json title,body --jq '"Issue #" + (.number|tostring) + ": " + .title + "\n\n" + .body' || echo "Issue context unavailable")
else
...
if gh_safe pr comment "$PR_NUMBER" --body-file "$ASSESSMENT_BODY_FILE" >/dev/null; then
...
if [ "$ACTIONABLE_LATER_COUNT" -gt 0 ]; then
  ...
  EXISTING_TECH_DEBT=$(gh_safe issue list --state open --label "tech-debt" --json number,title --jq '.[] | "\(.number):\(.title)"' || echo "")
  EXISTING_FOLLOWUPS=$(gh_safe issue list --state open --label "review-follow-up" --json number,title --jq '.[] | "\(.number):\(.title)"' || echo "")
  ...
  PR_LINKED_ISSUES=$(gh_safe pr view "$PR_NUMBER" --json body --jq '.body' | grep -oE "Follow-up.*#[0-9]+" | grep -oE "#[0-9]+" || echo "")
    ...
    case "$line" in
      ...
      ---END---)
        ...
        if [ -n "$DUPLICATE_ISSUE" ]; then
          ...
          EXISTING_BODY=$(gh_safe issue view "$DUPLICATE_ISSUE" --json body --jq '.body' || echo "")
...
_Added by Sharkrite on ${ASSESSMENT_TIMESTAMP}_"
            ...
            gh_safe issue edit "$DUPLICATE_ISSUE" --body-file "$EDIT_BODY_FILE" >/dev/null && \
...
_Parent PR: #${PR_NUMBER}_"
  ...
  if [ -n "$CREATED_ISSUES" ] || [ -n "$UPDATED_ISSUES" ]; then
    ...
    CURRENT_BODY=$(gh_safe pr view "$PR_NUMBER" --json body --jq '.body' || echo "")
...
- Updated: ${UPDATED_ISSUES:-none}"
    ...
    gh_safe pr edit "$PR_NUMBER" --body-file "$PR_EDIT_BODY_FILE" >/dev/null && \
